...
public class MainFrame extends JFrame {
    ...
    private void initFrame() {
        ...
        setSize(ui.LayoutConstants.FRAME_SIZE);
        setMinimumSize(new Dimension(1000, 700));
        getContentPane().setBackground(ui.Colors.LIGHT_BG);
    }

    private void initComponents() {
        // 顶部标题栏
        JPanel topPanel = new JPanel(new FlowLayout(FlowLayout.CENTER));
        topPanel.setBackground(ui.Colors.PRIMARY);
        topPanel.setPreferredSize(new Dimension(
                ui.LayoutConstants.FRAME_SIZE.width,
                ui.LayoutConstants.TOP_BAR_HEIGHT
        ));

        JLabel titleLabel = new JLabel("Linguistic Text Search Analysis ");
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, ui.LayoutConstants.FONT_H2));
        titleLabel.setForeground(ui.Colors.LIGHT_TEXT);
        topPanel.add(titleLabel);

        // 左侧面板组件
        JPanel leftPanel = new JPanel();
        leftPanel.setBackground(ui.Colors.LIGHT_BG);
        leftPanel.setPreferredSize(new Dimension(
                ui.LayoutConstants.SIDEBAR_WIDTH,
        ...
        searchButton.addActionListener(e -> {
            ...
            try {
                ...
                for (String sentence : results) {
                    if (sentence.contains(keyword) && index < resultAreas.length) {
                        resultAreas[index].setText(sentence);
                        index++;
                    }
                }

                // 清空剩余区域
                while (index < resultAreas.length) {
                    resultAreas[index].setText("");
                    index++;
                }
